...
// 수식 유효성 검사
bool validateExpression(const char* expr, const int len) {
    int parenCount = 0;
...
// 중위 표기법을 후위 표기법으로 변환
int infixToPostfix(const char* infix, char* postfix, const int len) {
    char stack[200];
    int top = -1;
    int j = 0;
...
// 후위 표기법 계산
double evaluatePostfix(const char* postfix, const int len, bool& divByZero) {
    double stack[200];
    int top = -1;
...
void processTestCase(char* buffer) {
    ...
    if (len > 100) {
        cout << "후위 표기 : " << endl;
        cout << "결과 값 : 테스트 케이스의 길이가 100을 초과합니다." << endl;
        cout << "************" << endl;
        return;
    }
